...
class ServerConfigReaderTests(LogCaptureTestCase):
	...
	if STOCK:
		...
		def testCheckStockCommandActions(self):
			...
			testJailsActions = (
				# iptables-multiport --
				('j-w-iptables-mp', 'iptables[name=%(__name__)s, bantime="10m", port="http,https", protocol="tcp", chain="INPUT"]', {
				...
				# iptables-allports --
				('j-w-iptables-ap', 'iptables[actiontype=<allports>][name=%(__name__)s, bantime="10m", protocol="tcp", chain="INPUT"]', {
					'ip4': ('`iptables ', 'icmp-port-unreachable'), 'ip6': ('`ip6tables ', 'icmp6-port-unreachable'),
					'start': (
				...
				('j-w-iptables-ipset', 'iptables-ipset-proto6[name=%(__name__)s, bantime="10m", port="http", protocol="tcp", chain="INPUT"]', {
					...
					'start': (
						"`ipset create f2b-j-w-iptables-ipset hash:ip timeout 600 family inet`",
				...
				('j-w-iptables-ipset-ap', 'iptables-ipset-proto6[actiontype=<allports>][name=%(__name__)s, bantime="10m", chain="INPUT"]', {
					'ip4': (' f2b-j-w-iptables-ipset-ap ',), 'ip6': (' f2b-j-w-iptables-ipset-ap6 ',),
					'start': (
						"`ipset create f2b-j-w-iptables-ipset-ap hash:ip timeout 600 family inet`",
				...
				# iptables --
				('j-w-iptables', 'iptables[actiontype=<dport>][name=%(__name__)s, bantime="10m", port="http", protocol="tcp", chain="INPUT"]', {
					'ip4': ('`iptables ', 'icmp-port-unreachable'), 'ip6': ('`ip6tables ', 'icmp6-port-unreachable'),
					'start': (
				...
				# iptables-new --
				('j-w-iptables-new', 'iptables[actiontype=<new>][name=%(__name__)s, bantime="10m", port="http", protocol="tcp", chain="INPUT"]', {
					'ip4': ('`iptables ', 'icmp-port-unreachable'), 'ip6': ('`ip6tables ', 'icmp6-port-unreachable'),
					'start': (
						"`iptables -w -N f2b-j-w-iptables-new`",
						"`iptables -w -A f2b-j-w-iptables-new -j RETURN`",
						"`iptables -w -I INPUT -m conntrack --ctstate NEW -p tcp -m multiport --dports http -j f2b-j-w-iptables-new`",
						"`ip6tables -w -N f2b-j-w-iptables-new`",
						"`ip6tables -w -A f2b-j-w-iptables-new -j RETURN`",
						"`ip6tables -w -I INPUT -m conntrack --ctstate NEW -p tcp -m multiport --dports http -j f2b-j-w-iptables-new`",
					),
					'stop': (
						"`iptables -w -D INPUT -m conntrack --ctstate NEW -p tcp -m multiport --dports http -j f2b-j-w-iptables-new`",
						"`iptables -w -F f2b-j-w-iptables-new`",
						"`iptables -w -X f2b-j-w-iptables-new`",
						"`ip6tables -w -D INPUT -m conntrack --ctstate NEW -p tcp -m multiport --dports http -j f2b-j-w-iptables-new`",
				...
				('j-w-fwcmd-mp', 'firewallcmd[name=%(__name__)s, bantime="10m", port="http,https", protocol="tcp", chain="INPUT"]', {
					...
					'start': (
						...
						"`firewall-cmd --direct --add-rule ipv4 filter INPUT 0 -p tcp -m multiport --dports http,https -j f2b-j-w-fwcmd-mp`",
						...
						"`firewall-cmd --direct --add-rule ipv6 filter INPUT 0 -p tcp -m multiport --dports http,https -j f2b-j-w-fwcmd-mp`",
					),
					'stop': (
						"`firewall-cmd --direct --remove-rule ipv4 filter INPUT 0 -p tcp -m multiport --dports http,https -j f2b-j-w-fwcmd-mp`",
						...
						"`firewall-cmd --direct --remove-rule ipv6 filter INPUT 0 -p tcp -m multiport --dports http,https -j f2b-j-w-fwcmd-mp`",
				...
				('j-w-fwcmd-ap', 'firewallcmd[actiontype=<allports>][name=%(__name__)s, bantime="10m", protocol="tcp", chain="INPUT"]', {
					'ip4': (' ipv4 ', 'icmp-port-unreachable'), 'ip6': (' ipv6 ', 'icmp6-port-unreachable'),
					'start': (
						...
						"`firewall-cmd --direct --add-rule ipv4 filter INPUT 0 -p tcp -j f2b-j-w-fwcmd-ap`",
						...
						"`firewall-cmd --direct --add-rule ipv6 filter INPUT 0 -p tcp -j f2b-j-w-fwcmd-ap`",
					),
					'stop': (
						"`firewall-cmd --direct --remove-rule ipv4 filter INPUT 0 -p tcp -j f2b-j-w-fwcmd-ap`",
						...
						"`firewall-cmd --direct --remove-rule ipv6 filter INPUT 0 -p tcp -j f2b-j-w-fwcmd-ap`",
				...
				('j-w-fwcmd-ipset', 'firewallcmd-ipset[name=%(__name__)s, bantime="10m", port="http", protocol="tcp", chain="INPUT"]', {
					...
					'start': (
						"`ipset create f2b-j-w-fwcmd-ipset hash:ip timeout 600 family inet`",
				...
				('j-w-fwcmd-rr', 'firewallcmd-rich[name=%(__name__)s, bantime="600", port="http,https", protocol="tcp", chain="INPUT_direct"]', {
					'ip4': (' ipv4 ', 'icmp-port-unreachable'), 'ip6': (' ipv6 ', 'icmp6-port-unreachable'),
					'start': (),
					'stop': (),
					'ip4-check': (),
					'ip6-check': (),
					'ip4-ban': (
						r"""`ports="http,https"; for p in $(echo $ports | tr ", " " "); do firewall-cmd --add-rich-rule="rule family='ipv4' source address='192.0.2.1' port port='$p' protocol='tcp'  reject type='icmp-port-unreachable'"; done`""",
					),
					'ip4-unban': (
						r"""`ports="http,https"; for p in $(echo $ports | tr ", " " "); do firewall-cmd --remove-rich-rule="rule family='ipv4' source address='192.0.2.1' port port='$p' protocol='tcp'  reject type='icmp-port-unreachable'"; done`""",
					),
					'ip6-ban': (
						r"""`ports="http,https"; for p in $(echo $ports | tr ", " " "); do firewall-cmd --add-rich-rule="rule family='ipv6' source address='2001:db8::' port port='$p' protocol='tcp'  reject type='icmp6-port-unreachable'"; done`""",
					),
					'ip6-unban': (
				...
				('j-w-fwcmd-rl', 'firewallcmd-rich[logging=on][name=%(__name__)s, bantime="600", port="http,https", protocol="tcp", chain="INPUT_direct"]', {
					'ip4': (' ipv4 ', 'icmp-port-unreachable'), 'ip6': (' ipv6 ', 'icmp6-port-unreachable'),
					'start': (),
					'stop': (),
					'ip4-check': (),
					'ip6-check': (),
					'ip4-ban': (
						r"""`ports="http,https"; for p in $(echo $ports | tr ", " " "); do firewall-cmd --add-rich-rule="rule family='ipv4' source address='192.0.2.1' port port='$p' protocol='tcp' log prefix='f2b-j-w-fwcmd-rl' level='warning' limit value='6/m' reject type='icmp-port-unreachable'"; done`""",
					),
					'ip4-unban': (
						r"""`ports="http,https"; for p in $(echo $ports | tr ", " " "); do firewall-cmd --remove-rich-rule="rule family='ipv4' source address='192.0.2.1' port port='$p' protocol='tcp' log prefix='f2b-j-w-fwcmd-rl' level='warning' limit value='6/m' reject type='icmp-port-unreachable'"; done`""",
					),
					'ip6-ban': (
						r"""`ports="http,https"; for p in $(echo $ports | tr ", " " "); do firewall-cmd --add-rich-rule="rule family='ipv6' source address='2001:db8::' port port='$p' protocol='tcp' log prefix='f2b-j-w-fwcmd-rl' level='warning' limit value='6/m' reject type='icmp6-port-unreachable'"; done`""",
					),
					'ip6-unban': (
						r"""`ports="http,https"; for p in $(echo $ports | tr ", " " "); do firewall-cmd --remove-rich-rule="rule family='ipv6' source address='2001:db8::' port port='$p' protocol='tcp' log prefix='f2b-j-w-fwcmd-rl' level='warning' limit value='6/m' reject type='icmp6-port-unreachable'"; done`""",
					),					
				}),
                # firewallcmd-new --
				('j-w-fwcmd-new', 'firewallcmd[actiontype=<new>][name=%(__name__)s, bantime="600", port="http,https", protocol="tcp", chain="INPUT_direct"]', {
					'ip4': (' ipv4 ', 'icmp-port-unreachable'), 'ip6': (' ipv6 ', 'icmp6-port-unreachable'),
					'start': (
						"`firewall-cmd --direct --add-chain ipv4 filter f2b-j-w-fwcmd-new`",
						"`firewall-cmd --direct --add-rule ipv4 filter f2b-j-w-fwcmd-new 1000 -j RETURN`",
						"`firewall-cmd --direct --add-rule ipv4 filter INPUT_direct 0 -m conntrack --ctstate NEW -p tcp -m multiport --dports http,https -j f2b-j-w-fwcmd-new`",
						"`firewall-cmd --direct --add-chain ipv6 filter f2b-j-w-fwcmd-new`",
						"`firewall-cmd --direct --add-rule ipv6 filter f2b-j-w-fwcmd-new 1000 -j RETURN`",
						"`firewall-cmd --direct --add-rule ipv6 filter INPUT_direct 0 -m conntrack --ctstate NEW -p tcp -m multiport --dports http,https -j f2b-j-w-fwcmd-new`",
					),
					'stop': (
						"`firewall-cmd --direct --remove-rule ipv4 filter INPUT_direct 0 -m conntrack --ctstate NEW -p tcp -m multiport --dports http,https -j f2b-j-w-fwcmd-new`",
						"`firewall-cmd --direct --remove-rules ipv4 filter f2b-j-w-fwcmd-new`",
						"`firewall-cmd --direct --remove-chain ipv4 filter f2b-j-w-fwcmd-new`",
						"`firewall-cmd --direct --remove-rule ipv6 filter INPUT_direct 0 -m conntrack --ctstate NEW -p tcp -m multiport --dports http,https -j f2b-j-w-fwcmd-new`",
						"`firewall-cmd --direct --remove-rules ipv6 filter f2b-j-w-fwcmd-new`",
						"`firewall-cmd --direct --remove-chain ipv6 filter f2b-j-w-fwcmd-new`",
					),
					'ip4-check': (
						r"`firewall-cmd --direct --get-chains ipv4 filter | sed -e 's, ,\n,g' | grep -q '^f2b-j-w-fwcmd-new$'`",
					),
					'ip6-check': (
						r"`firewall-cmd --direct --get-chains ipv6 filter | sed -e 's, ,\n,g' | grep -q '^f2b-j-w-fwcmd-new$'`",
					),
					'ip4-ban': (
						r"`firewall-cmd --direct --add-rule ipv4 filter f2b-j-w-fwcmd-new 0 -s 192.0.2.1 -j REJECT --reject-with icmp-port-unreachable`",
					),
					'ip4-unban': (
						r"`firewall-cmd --direct --remove-rule ipv4 filter f2b-j-w-fwcmd-new 0 -s 192.0.2.1 -j REJECT --reject-with icmp-port-unreachable`",
					),
					'ip6-ban': (
						r"`firewall-cmd --direct --add-rule ipv6 filter f2b-j-w-fwcmd-new 0 -s 2001:db8:: -j REJECT --reject-with icmp6-port-unreachable`",
					),
					'ip6-unban': (
						r"`firewall-cmd --direct --remove-rule ipv6 filter f2b-j-w-fwcmd-new 0 -s 2001:db8:: -j REJECT --reject-with icmp6-port-unreachable`",
					),					
				}),
			)
			server = TestServer()
